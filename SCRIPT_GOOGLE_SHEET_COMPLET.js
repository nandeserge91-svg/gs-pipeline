...
// 🔔 URL du webhook Make (optionnel, garder si vous voulez)
...
function testEnvoiVersGSPipeline() {
  const payload = {
    nom: 'Test Client Google Sheet',
    telephone: '+212600000000',
    ville: 'Casablanca',
    offre: 'Montre Connectée Pro',
    tag: 'test-google-sheet'
  };

  console.log('📤 Test envoi vers GS Pipeline...');
  ...
  console.log('Code réponse GS Pipeline :', responseCode);
  console.log('Contenu réponse GS Pipeline :', responseBody);
  
  if (responseCode === 200) {
    console.log('✅ SUCCÈS ! Allez dans GS Pipeline → À appeler pour voir la commande !');
  } else {
    console.error('❌ ERREUR ! Code :', responseCode);
  }
}
